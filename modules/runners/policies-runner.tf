resource "aws_iam_role" "runner" {
  name               = "${var.stack_name}-${var.prefix}-runner-role"
...
resource "aws_iam_instance_profile" "runner" {
  name = "${var.stack_name}-${var.prefix}-runner-profile"
...
resource "aws_iam_role_policy" "ssm_parameters" {
  ...
  policy = templatefile("${path.module}/policies/instance-ssm-parameters-policy.json",
    {
      arn_ssm_parameters_prefix = "arn:${var.aws_partition}:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter/${var.stack_name}-${var.prefix}-*",
      arn_ssm_parameters_path   = "arn:${var.aws_partition}:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter/${var.stack_name}-${var.prefix}/*",
      other_arn                 = "*"
    }
  )
}
