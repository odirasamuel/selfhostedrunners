...
resource "aws_iam_role_policy" "lambda_pool_vpc" {
  count = length(var.config.lambda.subnet_ids) > 0 && length([aws_security_group.runner_pool_sg.id]) > 0 ? 1 : 0
  name  = "${var.config.stack_name}-${var.config.prefix}-lambda-pool-vpc"
